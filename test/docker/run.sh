...
check_prerequisite() {
    docker --version > /dev/null || die "ERROR: docker is not installed"
    docker-compose --version > /dev/null || die "ERROR: docker-compose is not installed"
}

fetch_artifact() { # source_location output_path
     curl --silent --fail --netrc "$1" --output "$2" --create-dirs \
         || die "ERROR: unable to fetch artifact"
}
