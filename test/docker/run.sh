...
progress() { # message cmd [args]...
    ...
    echo
    wait "$pid" # return the exit code of the background process
}
...
fetch_artifact() { # source_location output_path
    progress "Fetching $1" curl --silent --fail --netrc "$1" --output "$2" \
        --create-dirs || die "unable to fetch $1"
}
...
run_events_plugin_tests() {
    ...
    docker exec --user=gerrit_admin "$runtests_container" \
            bash -c '/events/test/docker/run_tests/start.sh'
}
