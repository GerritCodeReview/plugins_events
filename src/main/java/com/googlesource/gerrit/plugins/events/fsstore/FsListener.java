...
public class FsListener implements Runnable {
  public static class FsLifecycleListener implements LifecycleListener {
    protected static final int INITIAL_DELAY_MS = 10000;
    protected final WorkQueue queue;
    protected final long pollingInterval = INITIAL_DELAY_MS;
    protected final DynamicSet<StreamEventListener> listeners;

    @Inject
    protected FsLifecycleListener(WorkQueue queue, DynamicSet<StreamEventListener> listeners) {
      //        @CleanupInterval long pollingInterval) {
      this.queue = queue;
    ...
    public void start() {
      queue
          .getDefaultQueue()
          .scheduleAtFixedRate(
              new FsListener(listeners), INITIAL_DELAY_MS, pollingInterval, MILLISECONDS);
    }
